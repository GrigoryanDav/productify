...
export const createComment = async (req: Request<CreateCommentParams>, res: Response) => {
  try {
...
export const deleteComment = async (req: Request<DeleteCommentParams>, res: Response) => {
  try {
    const { userId } = getAuth(req);
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
